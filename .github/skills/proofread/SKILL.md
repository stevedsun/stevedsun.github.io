---
name: proofread
description: 校对 Markdown 文章。当用户需要对 Hugo 博客文章的 Markdown 文件进行格式与语言校对时使用此技能。
---

## When to use

当用户需要对 Hugo 博客文章的 Markdown 文件进行格式与语言校对时使用。
...
4. 对全文执行详尽校对，输出：问题清单（行号 + 类型）、每处的修复建议、以及可直接 apply 的 patch 或完整修复后的 Markdown。
...
- 用户：检查 content/posts/2025/draft.md
