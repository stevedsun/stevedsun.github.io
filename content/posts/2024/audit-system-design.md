...
导出到第三方系统通常可以采用标准的 Syslog 格式或是 JSON Lines，支持范围最广。此外，需要考虑日志截断、第三方系统批处理和流处理的配置等，可以参考 kubernetes 的[这篇文档](https://kubernetes.io/zh-cn/docs/tasks/debug/debug-cluster/audit/#webhook-backend)。
...
![kubernetes log](/images/audit-system-design/kubernetes-audit.png)

[kubernetes 审计事件](https://kubernetes.io/zh-cn/docs/tasks/debug/debug-cluster/audit/) 使用和 Event API 不同的消息结构[^3]。
...
[^3]: [kubernetes 审计事件结构定义](https://kubernetes.io/zh-cn/docs/reference/config-api/apiserver-audit.v1/#audit-k8s-io-v1-Event)
...
- 优点：微服务设计，支持 kubernetes 等基础设施，多语言多平台提供了 SDK 和扩展能力。有完善的安全、完整性考虑。适合中小企业。
